...
// Resize background image to fit within browser viewing area
	function resizeMe(){
	// determine browser width/height and adjust main picture
...
/*
		if ((width >= 320) && (width <= 480) && (height < 640)) {
			height = height + 240;
		} // small devices (portrait) and screen

		else if ((width <= 1024) && (width > height) && (height < 600)) {
			height = 700;
			margintop = Math.round(height/4);
			mobile_flag = 0;
		} // if landscape

		else if (width <= 320) { height = height + 80; } // very small devices
		else if ((width < 620) && (height < 800) && (height > width)) { height = height - 90; margintop = Math.round(height/2.7); mobile_flag = 0; } // Nexus 7
		else { height = height - 90; margintop = Math.round(height-500); mobile_flag = 0; } // desktop
*/


		if (typeof height === "number") { height = height + "px"; }

		// set heights of image sections
		if (jQuery('#intro')) { jQuery("body #intro").css("height", 400); }
		if (jQuery('.bg')) { jQuery("body .bg").css("height", 400); }
		if (jQuery('#map')) { jQuery("body #map").css("height", 300); }

    	//var margintop = Math.round((height/3) - 250);
/*
		if (mobile_flag === 1) { jQuery("#google-maps").css("margin-top", "0px"); } // remove margin-top for mobile devices
		else { jQuery("#google-maps").css("margin-top", "0px"); } // revert to original margin-top upon resize
*/
...
// End resize background image

});

jQuery(function() {
    if(jQuery.ui) { window.alert('grrrrr');}
    jQuery('#intro a.alert,ul.nav-bar a,a.logo.left').bind('click', function(event) {
        ...
        if(anchor.attr('href') === '#') {
            ...
            topPosition = jQuery(anchor.attr('href')).offset().top-80;
        }
        //prevent browser from jumping directly to anchor, will fail back to regular nav without js
        ...
        //set the url to the new anchor position, to allow linking to sections of site
        location.hash = (anchor.attr('href'));
        //set home link url to '#' to allow easy nav to top without refreshing page.
        jQuery('a.logo.left').attr('href', '#')
    });
});
